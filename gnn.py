...
class AsyncGGNN(object):
    ...
    def debug(self,
        ...
        with tf.variable_scope('async_ggnn'):
            ...
            prop_round = 0
            self.hyperparams['propagation_substeps'] = 1
            with tf.variable_scope('prop_round%i' % (prop_round,)):
                ...
                for unlabeled_edge_typ in range(self.num_unlabeled_edge_types):
                    ...
                    sending_nodes_ta = sending_nodes_ta.write(
                        step * self.num_edge_types + shifted_edge_typ,
                        sending_nodes[prop_round][step][shifted_edge_typ])

                msg_targets_ta = msg_targets_ta.write(
                    step,
                    msg_targets[prop_round][step],
                )
                receiving_nodes_ta = receiving_nodes_ta.write(
                    step, receiving_nodes[prop_round][step])
                #receiving_node_num_ta = receiving_node_num_ta.unstack(
                #    receiving_node_num[prop_round])
                #receiving_node_num_ta = receiving_node_num_ta.write(
                #    step, receiving_node_num[prop_round])

                new_node_states_ta = tf.TensorArray(
                    tf.float32,
                    infer_shape=False,
                ...
                new_node_states_ta = new_node_states_ta.scatter(
                    initial_nodes[prop_round],
                    tf.gather(cur_node_states, initial_nodes[prop_round]))

                sending_states_per_edge_type = []
                #for unlabeled_edge_typ in range(self.num_unlabeled_edge_types):

                unlabeled_edge_typ = 0
                shifted_edge_typ = self.num_labeled_edge_types + unlabeled_edge_typ
                #sending_states_per_edge_type.append(
                #    new_node_states_ta.gather(
                #        sending_nodes_ta.read(
                #            substep_id * self.num_edge_types + shifted_edge_typ)
                #))
                sending_states_per_edge_type.append(
                    tf.gather(
                        new_node_states,
                ...
                old_receiving_node_states = tf.gather(
                    cur_node_states, substep_receiving_nodes)
                old_receiving_node_states.set_shape(
                    [None, self.hyperparams['hidden_size']])

                msg_targets_this_step = msg_targets_ta.read(substep_id)
                #receiving_node_num_this_step = \
                #    receiving_node_num_ta.read(substep_id)

                # Input looks like [receiving_node_nums]
                receiving_node_num_this_step = receiving_node_num[0][substep_id]
                ...
                #new_node_states_ta = new_node_states_ta.scatter(
                #    indices=substep_receiving_nodes,
                #    value=substep_new_node_states,
                #    name="state_scatter_round%i" % (prop_round,))

                new_node_states = tf.tensor_scatter_update(
                    tensor=new_node_states,
                    indices=tf.expand_dims(substep_receiving_nodes, 1),
                    updates=substep_new_node_states,
                    name="state_scatter_round%i" % (prop_round,))
                #return substep_id + 1, new_node_states_ta

                #_, new_node_states_ta = tf.while_loop(
                #    cond=is_done,
                #    body=do_substep,
                ...
                #cur_node_states = new_node_states_ta.stack(
                #    name="state_stack_round%i" % (prop_round,))
                cur_node_states = tf.stack(
                    new_node_states,
                    name="state_stack_round%i" % (prop_round,))


            #return substep_receiving_nodes
            return (cur_node_states, new_node_states)
            #return sending_states_per_edge_type[0]
            ...
            for prop_round in range(self.hyperparams['propagation_rounds']):
                with tf.variable_scope('prop_round%i' % (prop_round,)):
                    ...
                    def do_substep(substep_id, new_node_states):
                        ...
                        for unlabeled_edge_typ in range(self.num_unlabeled_edge_types):
                            ...
                            sending_states_per_edge_type.append(
                                tf.gather(
                                    new_node_states,
                                    sending_nodes_ta.read(
                                        substep_id * self.num_edge_types + \
                                        shifted_edge_typ)
                            ))
                        ...
                        #    receiving_node_num_ta.read(substep_id)
                        receiving_node_num_this_step = \
                            receiving_node_num[0][substep_id]
                        ...
                        new_node_states = tf.tensor_scatter_update(
                            ...
                            updates=substep_new_node_states,
                            name="state_scatter_round%i" % (prop_round,))
                        return substep_id + 1, new_node_states

                    def is_done(substep_id, new_node_states_ta_unused):
                        return tf.logical_and(
                            substep_id < self.hyperparams['propagation_substeps'],
    ...
    def propagate_one_step(self,
        ...
        for (edge_typ, sending_state_representations) in enumerate(sending_states_per_edge_type):
            if edge_typ < self.num_labeled_edge_types:
                ...
                messages = tf.matmul(
                    sending_state_representations,
                    self.__parameters['unlabeled_edge_weights'][shifted_edge_typ]
                )
                if self.hyperparams['use_edge_bias']:
