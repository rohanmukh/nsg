...
class Model:
    def __init__(self, config, top_k=5):
        ...
        self.ast_gen_loss_op_matrix, self.ast_gen_loss_op = get_loss(5, OP_NODE)
        self.ast_gen_loss_method_matrix, self.ast_gen_loss_method = get_loss(6, METHOD_NODE)
        ############################

        self.ast_gen_loss = self.ast_gen_loss_method + self.ast_gen_loss_op + \
    ...
    def get_decoder_probs(self, sess, nodes, edges, targets, var_decl_ids, ret_reached, \
        ...
        def calculate_mean_prob(matrix, targets, node_type_number, node_type):
            # This implementation counts number of times the next token is selected
            # with highest probability. The original prob is NOT returned
        ...
        return [concept_mean_prob, api_mean_prob, type_mean_prob,
                clstype_mean_prob, var_mean_prob, op_mean_prob,
                method_mean_prob]

    def get_horizon_probs(self, sess, nodes, edges, targets, var_decl_ids, ret_reached, node_type_number,
                          type_helper_val, expr_type_val, ret_type_val, all_var_mappers, iattrib, apis, types, kws,
                          return_type, formal_param_inputs,
                          fields, method, classname, javadoc_kws,
                          surr_ret, surr_fp, surr_method,
                          visibility=1.00, ignore_concepts=True
                          ):

        # from scipy.special import softmax
        def update_keys(parent_dict, new_dict):
            for len in new_dict.keys():
                count, curr_sum, curr_real_sum = new_dict[len]
                if len not in parent_dict:
                    parent_dict[len] = [0, 0.0, 0.0]
                parent_dict[len][0] += count
                parent_dict[len][1] += curr_sum
                parent_dict[len][2] += curr_real_sum

        def calculate_horizon_prob(matrix, targets, node_type_number, node_type, time_varying_agg_prob, all_ast_ln_probs,
                                   ):
            matrix_choices = np.argmax(matrix, axis=2)

            ast_probs = np.sum((matrix_choices == targets) * (node_type_number == node_type), axis=0)
            matrix_probs = np.sum((matrix_choices == targets) * (node_type_number == node_type)
                                  * np.exp(all_ast_ln_probs[node_type])
                                  , axis=0)

            num_nodes = np.sum(node_type_number == node_type, axis=0)

            time_varying_prob = dict()
            for j in range(targets.shape[1]):# time index
                num_node = num_nodes[j]
                prob = ast_probs[j]
                real_prob = matrix_probs[j]
                time_varying_prob[j] = [num_node, prob, real_prob]
            update_keys(time_varying_agg_prob, time_varying_prob)
        ...
        ast_logits, all_ast_probs = sess.run([self.decoder.ast_logits, self.all_ast_ln_probs], feed)

        concept_matrix, api_matrix, type_matrix, clstype_matrix, var_matrix, op_matrix, method_matrix, _ = ast_logits

        time_varying_agg_prob = dict()  # each element is length -> [count, *cumul_sum]
        if not ignore_concepts:
            calculate_horizon_prob(concept_matrix, targets, node_type_number, CONCEPT_NODE, time_varying_agg_prob, all_ast_probs)
        calculate_horizon_prob(api_matrix, targets, node_type_number, API_NODE, time_varying_agg_prob, all_ast_probs)
        calculate_horizon_prob(type_matrix, targets, node_type_number, TYPE_NODE, time_varying_agg_prob, all_ast_probs)
        calculate_horizon_prob(clstype_matrix, targets, node_type_number, CLSTYPE_NODE, time_varying_agg_prob, all_ast_probs)
        calculate_horizon_prob(var_matrix, targets, node_type_number, VAR_NODE, time_varying_agg_prob, all_ast_probs)
        calculate_horizon_prob(op_matrix, targets, node_type_number, OP_NODE, time_varying_agg_prob, all_ast_probs)
        calculate_horizon_prob(method_matrix, targets, node_type_number, METHOD_NODE, time_varying_agg_prob, all_ast_probs)

        return time_varying_agg_prob
