...
MAJOR_CONCEPTS = [DAPIInvoke.name(), DClsInit.name(), DFieldDecl.name(), DVarDeclCls.name(),
                  DVarDecl.name(), DVarAssign.name(), DInfix.name(),
                  DReturnVar.name(), DStop.name(), DExceptionVarDecl.name(), #DAPICallSingle.name(),
                  DBranch.name(),  DLoop.name(), DExcept.name(),  DInternalAPICall.name(),
                  ]
