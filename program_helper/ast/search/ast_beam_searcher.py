...
from synthesis.ops.candidate_ast import Candidate_AST, CONCEPT_NODE, VAR_NODE, API_NODE, TYPE_NODE, SYMTAB_MOD, OP_NODE, \
    METHOD_NODE, CLSTYPE_NODE, NODETYPE_MAPPERS
...
from program_helper.ast.ops import CHILD_EDGE, SIBLING_EDGE, \
    ...
    DVarAssign, DCatch, DAPICallMulti, DAPICallSingle, SINGLE_PARENTS, DVarDeclCls, MAJOR_CONCEPTS
...
class TreeBeamSearcher:
    ...
    def __init__(self, infer_model):
        ...
        self.infertime_horizon_probs = dict()
        return


    def print_infer_horizon_stat(self, only_cumulatives=True, ignore_concepts=True):
        types = [CONCEPT_NODE, API_NODE, TYPE_NODE, CLSTYPE_NODE, VAR_NODE, OP_NODE,  SYMTAB_MOD, METHOD_NODE]
        all_type_node_dict = dict()
        all_but_concept_node_dict = dict()
        for type_ in types:
            if type_ not in self.infertime_horizon_probs:
                continue
            result_dict = self.infertime_horizon_probs[type_]
            for key in sorted(result_dict.keys()):
                count, cumul_sum = result_dict[key]
                if key not in all_type_node_dict:
                    all_type_node_dict[key] = [0, 0.]
                all_type_node_dict[key][0] += count
                all_type_node_dict[key][1] += cumul_sum

                if type_ != CONCEPT_NODE:
                    if key not in all_but_concept_node_dict:
                        all_but_concept_node_dict[key] = [0, 0.]
                    all_but_concept_node_dict[key][0] += count
                    all_but_concept_node_dict[key][1] += cumul_sum
                if not only_cumulatives:
                    print("For type {} at distance {} prob is {}".format(NODETYPE_MAPPERS[type_], key, cumul_sum / count))
            if not only_cumulatives:
                print()

        if not ignore_concepts:
            for key in sorted(all_type_node_dict.keys()):
                count, cumul_sum = all_type_node_dict[key]
                print("For type ALL-NODEs at distance {} prob is {}".format(key, cumul_sum / count))
            print()

        for key in sorted(all_but_concept_node_dict.keys()):
            count, cumul_sum = all_but_concept_node_dict[key]
            print("For type ALL-but-CONCEPT-NODEs at distance {} prob is {}".format(key, cumul_sum / count))
        print()
    ...
    def resolve_beam_probs(self, candies, all_beam_ln_probs):
        ...
        for k, candy in enumerate(candies):
            ...
            if candy.next_node_type not in self.infertime_horizon_probs:
                self.infertime_horizon_probs[candy.next_node_type] = dict()
            elif candy.length not in self.infertime_horizon_probs[candy.next_node_type]:
                probs = np.exp(all_beam_ln_probs[candy.next_node_type][k])
                max_prob = np.max(probs)
                self.infertime_horizon_probs[candy.next_node_type][candy.length] = [1, max_prob]
            else:
                count, curr_prob = self.infertime_horizon_probs[candy.next_node_type][candy.length]
                probs = np.exp(all_beam_ln_probs[candy.next_node_type][k])
                max_prob = np.max(probs)
                self.infertime_horizon_probs[candy.next_node_type][candy.length] = [count+1, curr_prob + max_prob]
    ...
    def get_next_output_with_fan_out(self, candies):
        ...
        for i, candy in enumerate(candies):
            if candy.next_node_type == CONCEPT_NODE:
                if candy.fixed_next_node is not None:
                    for j in range(self.beam_width):
                        if next_nodes[i][j] not in candy.fixed_next_node:
                            beam_ln_probs[i][j] = -np.inf
                else:
                    for j in range(self.beam_width):
                        if next_nodes[i][j] not in MAJOR_CONCEPTS:
                            beam_ln_probs[i][j] = -np.inf
            # elif candy.next_node_type == VAR_NODE:
            #     max_beam_ln_probs_i = max(beam_ln_probs[i])
            #     for j in range(self.beam_width):
            #         if beam_ln_probs[i][j] != max_beam_ln_probs_i:
            #             beam_ln_probs[i][j] = -np.inf
        ...
        for row, col in zip(rows, cols):
            ...
            if new_candy.is_rolling() and not np.isinf(beam_ln_probs[row][col]):
    ...
    def handle_control_flow(self, new_candy):
        ...
        elif node_value == DVarDeclCls.name():
            new_candy = self.handle_DVarDeclCls(new_candy, node_value=node_value)
    ...
    def handle_DAPICallSingle(self, new_candy):
        if not new_candy.stop_apisingle and new_candy.single_api_count <= 5:
            ...
            new_candy.control_flow_stack.append(CandidateElement(curr_node=new_candy.tree_currNode,
                                                                 curr_node_val=DAPICallSingle.name(),
                                                                 fixed_node_val=[DAPICallSingle.name(), DStop.name()],
                                                                 edge_path=[SIBLING_EDGE],
                                                                 next_node_type=CONCEPT_NODE
                                                                 )
                                                )
    ...
    def handle_DVarDeclCls(self, new_candy, node_value=DVarDecl.name()):
        # last we will expand the elements after invoke
        new_candy.control_flow_stack.append(CandidateElement(curr_node=new_candy.tree_currNode,
                                                             curr_node_val=node_value,
                                                             edge_path=[SIBLING_EDGE],
