...
class JSON_Synthesis:
    ...
    def expand_all_siblings_till_STOP(self, json_nodes, head_node):
        ...
        while not (isinstance(head_node, DStop) or (head_node is None)):
            ...
            # DVarDeclCls comes first since it is inheritd from DVarDecl
            elif isinstance(node, DVarDeclCls):
                # DVarDeclCls inherited from here
                json_nodes.append({'node': DVarDeclCls.name(),
                ...
                astnode['_call'] = node.child.child.val
                astnode['_id'] = node.child.sibling.val
